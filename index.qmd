...
# DAG 1: The 1747 Understanding - Lemons prevent scurvy
pgm1 = daft.PGM(dpi=150)

# Add nodes with professional styling
pgm1.add_node("lemons", "Lemons", 1, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFE66D',  # Lemon yellow
                 'edgecolor': '#FFA500',   # Orange border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

pgm1.add_node("scurvy", "Scurvy", 3.5, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#98D8C8',  # Soft teal (health)
                 'edgecolor': '#2B7A78',   # Dark teal border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

# Add edge showing causal relationship
pgm1.add_edge("lemons", "scurvy")

# Render the DAG
pgm1.render()
plt.title("1747 Understanding: Lemons Prevent Scurvy", fontsize=14, pad=20)
```
#### Understanding 
- **Empirical knowledge can work without understanding (but is fragile)**
...
# DAG 2: The Misguided Belief - Acid kills bacteria (DIAGONAL LAYOUT)
pgm2 = daft.PGM(dpi=150)

# Add nodes for the incorrect causal chain - arranged diagonally (top-left to bottom-right)
pgm2.add_node("acid", "Acid", 1, 3, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFB3B3',  # Light red (warning)
                 'edgecolor': '#CC0000',   # Dark red border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

pgm2.add_node("bacteria", "Bacteria", 2, 2, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#D4A5A5',  # Dusty rose
                 'edgecolor': '#8B0000',   # Dark red border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

pgm2.add_node("scurvy", "Scurvy", 3, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#FFD4D4',  # Very light red
                 'edgecolor': '#B22222',   # Firebrick border
                 'linewidth': 2,
                 'alpha': 0.85,
             })
...
# Render the DAG
pgm2.render()
plt.title("Misguided Belief: Acid Kills Bacteria → Prevents Scurvy", fontsize=14, pad=20)
```

#### Understanding 
- **Wrong models are dangerous - they lead to wrong interventions**
...
# DAG 3: The 1928 Understanding - Vitamin C prevents scurvy (correct!)
pgm3 = daft.PGM(dpi=150)

# Add nodes with professional scientific styling - different vibrant colors
pgm3.add_node("vitamin_c", "Vitamin C", 1, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#9B59B6',  # Rich purple
                 'edgecolor': '#6C3483',   # Dark purple border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

pgm3.add_node("scurvy", "Scurvy", 3.5, 1, aspect=2.5, scale=1.2,
             plot_params={
                 'facecolor': '#3498DB',  # Bright blue
                 'edgecolor': '#1A5490',   # Navy blue border
                 'linewidth': 2,
                 'alpha': 0.85,
             })

# Add edge showing the correct causal relationship
pgm3.add_edge("vitamin_c", "scurvy")

# Render the DAG
pgm3.render()
plt.title("1928 Understanding: Vitamin C Prevents Scurvy (Correct!)", fontsize=14, pad=20)
```

#### Understanding 
- **True understanding enables robust, generalizable solutions**
